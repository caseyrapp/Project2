...
function Weight(pos) {
	this.basePos = this.pos = pos.plus(new Vector(0.1, 0.2));
	this.size = new Vector(0.6, 0.6);
	this.wobble = Math.random() * Math.PI * 2;
}	
Weight.prototype.type = "weight";
...
Weight.prototype.act = function(step) {
	this.wobble += step * wobbleSpeed;
	var wobblePos = Math.sin(this.wobble) * wobbleDist;
	this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin" || "boost" || "weight") {
    this.actors = this.actors.filter(function(other) {
      if (type == "boost") {
		  gravity = 17;
		  setTimeout(function(){gravity = 30;}, 5000);
	  }
	  if (type == "weight") {
		  playerXSpeed = 3;
		  setTimeout(function(){playerXSpeed = 7;}, 7000);
	  }	  
	  return other != actor;
  });
